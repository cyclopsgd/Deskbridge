...
- [ ] **RELY-01**: User can connect to an RDP host without being prompted to verify the server certificate
- [ ] **RELY-02**: User sees the tab automatically close when they log off from the remote session
- [ ] **RELY-03**: User's stored credentials are used automatically without unnecessary prompts when credential inheritance has no parent

#### Import
- [ ] **IMP-01**: User's mRemoteNG imported connections retain their usernames from the source XML
- [ ] **IMP-02**: User sees a loading indicator during import and the confirm button is disabled; subsequent clicks are ignored while processing

#### Stability & Performance
- [ ] **STAB-01**: User can delete multiple connections at once without the application crashing
- [ ] **STAB-02**: User sees no black flash when switching between tabs rapidly
- [ ] **STAB-03**: User sees RDP sessions rendered at native resolution matching their monitor DPI, reducing blur compared to mRemoteNG
- [ ] **STAB-04**: User sees no grey border around the VM viewport on work machines (investigation -- may be Group Policy)
- [ ] **STAB-05**: User experiences smooth scrolling and expand/collapse in the connection tree with 100+ connections (TreeView virtualization)

#### UX Improvements
- [ ] **UX-01**: User changes their connection password via a "Change Password" button with confirmation field instead of a bare password input
- [ ] **UX-02**: User can adjust text size in an Appearance setting to prevent text truncation on 1080p screens
- [ ] **UX-03**: User can detach a tab into a floating popout window and re-dock it (stretch goal)
...
- [x] **PERF-03
**: User sees the application start without UI freeze when loading a connections.json with 500+ entries (async JSON load)
- [x] **PERF-04
...
- [x] **SET-01
**: User sees a dedicated Settings page with categorized sections (Appearance, Security, Bulk Operations, Uninstall)
- [x] **SET-02
**: User can configure bulk operation preferences (confirm before bulk connect, GDI warning threshold)
- [x] **SET-03
...
- [x] **BULK-02
**: User can right-click a group or use a command to disconnect all active sessions in the group
- [x] **BULK-03
...
- v1.2 requirements: 13 total (all mapped)
- v1.3 requirements: 15 total (all mapped)
- v1.3 mapped to phases: 15/15

---
*Requirements defined: 2026-04-11*
*Last updated: 2026-04-25 after v1.3 roadmap creation*
