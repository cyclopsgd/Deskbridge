...
- **`docs/REFERENCE.md`** — Architecture, constraints, feature spec, disposal sequences, DI registrations
- **`docs/DESIGN.md`** — WPF-UI patterns, control usage, colour tokens, layout reference
- **`docs/WPF-UI-PITFALLS.md`** — 8 categories of silent WPF-UI failures with correct patterns. **Read this before writing ANY WPF-UI code.** Covers ContentDialog hosting, TreeView context menus, FluentWindow conflicts, icon resolution, Color vs Brush resource keys, MVVM command generation, and theme override persistence.
- **`docs/WPF-TREEVIEW-PATTERNS.md`** — **Read before TreeView code.** Multi-select attached behaviors, row-level selection visuals, collapsible panel patterns with persistent toggle headers.
- **`docs/RDP-ACTIVEX-PITFALLS.md`** — **Read before RDP Integration.** AxMsRdpClient9 siting/disposal, GDI handle leaks, IMsTscNonScriptable password casting, thread affinity, focus stealing, reconnection edge cases.
- **`docs/WINFORMS-HOST-AIRSPACE.md`** — **Read before RDP + Tab Management.** Airspace problem with FluentWindow/Mica, WM_ENTERSIZEMOVE bitmap snapshot, PerMonitorV2 DPI across monitors, WPF-UI overlay z-order against WindowsFormsHost.

All of these docs live in the `docs/` directory. If you're about to touch ContentDialog/ContextMenu/SymbolIcon/brushes — consult `docs/WPF-UI-PITFALLS.md`. Touching a TreeView — `docs/WPF-TREEVIEW-PATTERNS.md`. Touching ActiveX or WindowsFormsHost — the RDP docs.

- **`.claude/skills/deskbridge-design/`** — **Read before any UI/visual work.** Complete design system with Windows 11 Fluent tokens, spacing grid, component specs, hover/selected/press states, and icon usage. This is the visual authority for v1.1 UI Polish and all future UI changes. The `SKILL.md` is auto-loaded by Claude Code; the `README.md` has the full spec.
